 ...
 |-----Collection接口：单列集合，用来存储一个一个对象
        |-----Set接口：【无序】【不可重复】  --->高中讲的“集合”
                |----HashSet：作为Set接口的主要实现类；线程不安全的；可以存储null值
                        |----LinkedHashSet：作为HashSet的子类；遍历其内部数据及时，可以按照添加的顺序遍历
...
public class SetTest {
    ...
    /*
        一、Set接口：【无序】【不可重复】
        以HashSet为例说明：
        1.【无序】：不等于随机性。存储的数据在底层数组中并非按照数组索引的顺序添加。
                                而是根据数据的哈希值决定的。
        ...
        二、添加元素的过程：以HashSet为例：
            ...
            判断数组此位置上是否有元素：
             ...
             对于添加成功的情况2和情况3而言：元素 a 与已经存在指定索引位置上的数据以【链表】的方式存储。
