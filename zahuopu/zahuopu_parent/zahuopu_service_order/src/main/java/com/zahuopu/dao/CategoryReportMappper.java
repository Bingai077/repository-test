...
/**
 * @author bingai
 * @create 2020-01-15 9:06
 */
public interface CategoryReportMappper extends Mapper<CategoryReport> {

    @Select("SELECT " +
            "oi.category_id1 categoryId1,oi.category_id2 categoryId2,oi.category_id3 categoryId3,DATE_FORMAT(o.pay_time,'%Y-%m-%d') countDate, " +
            "SUM(oi.num) num, " +
    ...
    @Select("SELECT category_id1 categoryId1,c.`name` categoryName,SUM(num) num, SUM(money) money " +
            ...
            "AND count_date>=#{date1} AND count_date<=#{date2} " +
